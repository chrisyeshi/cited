<template>
  ...
  <!-- <v-container pb-1 :py-2="$store.state.isVisPaneVisible" px-0>
    <v-layout row wrap>
      <v-flex :md2="!$store.state.isVisPaneVisible" px-0>
        <v-layout row wrap justify-space-between px-0>
          <v-btn
      ...
      <v-flex :md10="!$store.state.isVisPaneVisible" :style="{ height: $store.state.isVisPaneVisible ? 'calc(100vh - 135px)' : 'calc(100vh - 100px)', overflow: 'auto', paddingLeft: '32px', paddingRight: '32px' }">
        <h2 class="mb-2 display-1">{{ refObj.title }}</h2>
        <h4 class="mb-2 subheading">
          <template v-for="(name, index) in authorNames">
            <a :key="`name-${index}`" class="text-no-wrap">{{ name }}</a>
